...
function reference_nodes(::Type{<:LagrangeLine{Np}}) where {Np}
    equis_nodes = LinRange(0, 1, Np)
    return [equis_nodes[1]; equis_nodes[end]; equis_nodes[2:(end-1)]]
end

@generated function (el::LagrangeLine{Np})(u) where {Np}
    basis_functions = compute_line_basis(el)
    quote
        v = vals(el)
        lag = zero(v[1])
        for i in 1:Np
            lag += v[i]*($basis_functions)[i](u)
        end
        return lag
    end
end

function compute_line_basis(T::Type{<:LagrangeLine{Np}}) where {Np}
    n = order(T)
    nodes = @memoize reference_nodes(T)
    basis_functions = map(eachindex(nodes)) do j
        xⱼ = nodes[j]
        basis = let n = n, nodes = nodes, xⱼ = xⱼ, j = j
            function (x)
                its = Iterators.filter(!isequal(j), 1:(n+1))
                denom = prod(k -> (xⱼ - nodes[k]), its; init = 1.0)
                numer = prod(k -> (x - nodes[k]), its; init = 1.0)
...
@generated function (el::LagrangeElement{ReferenceSimplex{2},Np})(u) where {Np}
    # precompute lagrange basis
    basis_functions = compute_simplicial_basis(el)
    quote
        v = vals(el)
        lag = zero(v[1])
        for i in 1:Np
...
function compute_simplicial_basis(T::Type{<:LagrangeTriangle{Np}}) where {Np}
    k = order(T)
    nodes = @memoize reference_nodes(T)
    basis_functions = map(nodes) do node
...
# The following specialized routines are faster than the generic Pk code (that
# are applicable for arbitrary k).

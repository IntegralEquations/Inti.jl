...
function (el::LagrangeLine{Np})(u) where {Np}
    _, basis_functions = compute_line_basis(typeof(el))
    v = vals(el)
    lag = zero(v[1])
...
function (el::LagrangeElement{ReferenceSimplex{2},Np})(u) where {Np}
    ...
    _, basis_functions = compute_simplicial_basis(typeof(el))
    v = vals(el)
    lag = zero(v[1])
    for i in eachindex(v)
...
# The following specialized routines are faster than the generic Pk code for
# arbitrary k

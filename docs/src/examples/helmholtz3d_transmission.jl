...
# ╔═╡ fbc8c8af-9825-496c-a620-644914569805
begin
	using Markdown
	using InteractiveUtils
end

# ╔═╡ 5ee9543e-b620-4403-94b7-deb7ea9576b5
begin
    import Pkg as _Pkg
...
# ╔═╡ 5cd4ca26-e552-4e2d-96a4-1eb66ce481b6
begin
    using Inti
    using LinearAlgebra
    using StaticArrays
...
# ╔═╡ 0d7a39b6-9511-11ef-1544-e944b826872d
md"""
# Helmholtz three-dimensional transmission boundary-value problem
...
# ╔═╡ c03ca6ef-21d3-4cdd-ad2c-94efe346c4f6
md"""
In this tutorial, we solve the three-dimensional Helmholtz transmission problem using Müller's indirect formulation.

Specifically, we focus on the scattering and transmission of an incident field given by a plane wave
```math 
...
# ╔═╡ 2244afd9-b585-4598-9dbe-e3ca03858996
md"""
...
# ╔═╡ d5c5b91f-37c8-4169-8140-5bbafc733c56
begin
	k₁       = 3π
	k₂       = 2π
	λ₁       = 2π / k₁
...
# ╔═╡ a18f00de-5d5f-42cd-88c9-515dc14b383c
md"""
## Geometry of the problem

We begin by defininf the geometry of the scatterer. For simplicity, we choose the unit sphere. Furthermore, we define additional planes that intersect the sphere in order to evaluate the scattered and transmitted fields.
"""

# ╔═╡ 40745461-58a4-45f0-b511-525e2c6dcc2d
begin
...
# ╔═╡ d0d97378-04da-4919-86c9-f084be833d5a
md"""
As customary, we will define the mesh size that is proportional to the wavelength:
"""

# ╔═╡ 13d4f54e-828a-4ec1-9372-3c6a569aec53
begin
...
# ╔═╡ 151f26a0-67a7-4d63-8bca-603d03f09e72
md"""
With the meshed geometry, we now proceed to define the boundary of the domain and the quadrature points.
"""

# ╔═╡ b7a49bb5-a15d-445e-b386-01ed33b07320
begin
...
# ╔═╡ 7b3c162d-8e13-4901-8980-0fae4476191b
begin
...
# ╔═╡ 986e507d-3740-46bc-991a-c99e38f523d1
md"""
## Indirect formulation

As done previously in two dimensions, we start by assuming that the scattered and transmitted fields are given by the following ansatz:
...
where $\mathcal D_i, \mathcal S_i$ are respectively the double- and single-layer potentials associated to the wavenumber $k_i, i=1,2$, previously defined in the two-dimensional case, and the densities ``\varphi,\psi:\Gamma\to\mathbb C`` are sufficiently regular functions. From our ansatz, taking the Dirichlet traces of fields provides on $\Gamma$

```math
\begin{aligned}
\gamma^+u_1 &= \frac12\varphi + D_1[\varphi] - S_1[\psi],\\
\gamma^-u_2 &= -\frac12\varphi + D_2[\varphi] - S_2[\psi].
\end{aligned}
```

Moreover, by taking the Neumann traces of our ansatz representation it follows that

```math
\begin{align}
\partial_n^+u_1=& \frac12\psi + N_1[\varphi] - K_1[\psi],\\
\partial_n^-u_2=& -\frac12\psi + N_2[\varphi] - K_2[\psi]
\end{align}
```

on $\Gamma$. The integral operators ``S_j, D_j, K_j``, and ``N_j``, are respectively as the single-layer, double-layer, adjoint double-layer, and hypersingular operators defined in our previous two-dimensional transmission problems. By substracting the previous corresponding trace representations, the following equations arise
...
Finally, similarly as done in the two-dimensional case, we can recast the BIE system:

```math
\left(I+\begin{bmatrix}D_1-D_2&-S_1+S_2\\N_1-N_2&-K_1+K_2\end{bmatrix}\right)\begin{bmatrix}\varphi\\ \psi \end{bmatrix} = \boldsymbol{b}.
```
where ``I`` above denotes the indentity operator. In what follows we use `Inti.jl` to define the external and internal PDEs and build the layer operators to construct the BIE system.

"""

# ╔═╡ 0fbc402e-17cc-43a8-bc3c-dc0e30bdedbf
md"""
## Test problem

In order to test the accuracy of the proposed formulation, we can manufacture solution fields for the transmission problem and construct the right-hand-side of the BIE system as the boundary information from the solution as

```math
\boldsymbol b= \begin{bmatrix}\gamma^+u_1 - \gamma^-u_2\\ \partial_n^+u_1 - \partial_n^-u_2\end{bmatrix}.
```

"""

# ╔═╡ 9f192ed7-7f13-4695-92d3-1813c9f9be8c
begin
...
# ╔═╡ cca1a0ce-867b-49c7-afe3-147243adc32b
md"""
We now solve the linear system using GMRES.
"""

# ╔═╡ 6c738a5b-5fdd-4632-af5a-d681a911f011
begin
	sol, hist =
...
# ╔═╡ 2b194d51-6cf0-4dcc-b291-78061a43029a
md"""
Once the linear system solution has been computed, we can define the solution to the transmission problem through our layer potential representation ansatz.
"""

# ╔═╡ c946c467-6022-4bec-aa9b-83405151a46d
begin
	nQ = size(Q, 1)
...
# ╔═╡ 83dedf7a-2f81-466b-8b42-06fa2c7e1b65
md"""
To check the accuracy of the numerical approximation, we can compute the error over points in spheres centered at the origin outside and inside the scatterer
"""

# ╔═╡ 15cfd157-5c38-465b-91ad-6a8dc3a15a00
begin
...
# ╔═╡ dbd6231b-9d3c-4ca7-9165-e7ce3d9c78d4
begin
		er₂ = maximum(1:100) do _
	    x̂ = rand(Inti.Point3D)|> normalize # an SVector of unit norm
	    x = 0.25 * x̂
...
# ╔═╡ c5cc1840-3512-4882-8715-f0f32f2177b7
md"""
## Visualization of the transmission problem

Once the accuracy of the proposed formulation has been tested, we can now redefine the right-hand-side of the BIE system so that we can now visualize the solution fields when the only known datum is the incident field. Namely, from the boundary conditions, we have that

```math
\begin{aligned}
\gamma^+u^{inc} &= \gamma^-u_2 - \gamma^+u_1, \\
\partial_n^+u^{inc} &= \partial_n^-u_2 - \partial_n^+u_1
\end{aligned}
```

over $\Gamma$. Hence, by defining

```math
\boldsymbol b= \begin{bmatrix}- \gamma^+u^{inc}\\ -\partial_n^+u^{inc}\end{bmatrix},
```

the BIE system's solution will now provide the solution fields for a known incident field. For this example, we consider an incident field described by a plane-wave and use it to define the right-hand-side of our BIE linear system.

"""

# ╔═╡ 7f03535a-07a1-47af-b98b-93d82f84e1c8
begin
...
# ╔═╡ 3ac4fa1d-5a82-47b7-b044-0b29029356e3
md"""
The solution vector is reshaped to represent the evaluations of the densities at the quadrature points.
"""

# ╔═╡ cbd6538c-2d4d-489d-ab29-e6a344ae3146
begin
	sol_temp_inc = reshape(sol_inc, nQ, 2)
	φ_inc, ψ_inc = sol_temp_inc[:, 1], sol_temp_inc[:, 2]
end

# ╔═╡ 6b040390-6d0f-403a-8c42-894b24449d8d
md"""
In order to visualize the solution fields, we use the previously defined `PhysicalGroups` as target points to evaluate the scattered and transmitted fileds using their ansatz layer potential representation. 
"""

# ╔═╡ 24f5bfe0-59b9-4fa2-b143-78694c4960e7
begin
...
# ╔═╡ 21a72a95-cff8-49e2-94cb-90c160bfc61b
md"""
Notice that since we have a relatively large number of target evaluation points, the potentials are constructed as operators in order to benefit from the compression and correction methods to obtain a faster and more accurate evaluation.
"""

# ╔═╡ 20a79e2c-3ed9-4d90-8ca2-995364b32387
begin
	S⁺, D⁺ = Inti.single_double_layer(;
	    pde = pde₁,
	    target = target_out,
...
# ╔═╡ d7902908-5c12-4b4d-888d-349b95ed34bf
md"""
Tovisualize the total field in the outside of the domain, we need the evaluation of the incident field in the external target points:
"""

# ╔═╡ 11f6bd88-174c-4660-8f93-2af50307c8a7
u_inc_out = u_inc.(target_out);

# ╔═╡ 40e86c65-1ab3-46ad-8207-fd05d9513b5a
md"""
Finally, we can visualize the solution for the Helmholtz transmission problem for an incident plane-wave field in three dimensions.
"""

# ╔═╡ e86f041c-4bb3-49d9-aef0-8def64a8164f
begin
	nv = length(Inti.nodes(Γ_msh))
	colorrange_out = extrema(real(w₁)+real(u_inc_out))
	...
	viz!(Σ⁺_msh; colorrange, colormap, color = real(w₁)+real(u_inc_out))
...
manifest_format = "2.0"
project_hash = "d80e93ed3e0e4ea8927b68b8d1744414a8d98b08"
...
git-tree-sha1 = "7339fd7714bb1fea9cc6c530f255980cba64b4f4"
...
git-tree-sha1 = "c4ec1675cf7a03048c0b2d4c429c1c8ef24ae06b"
...
git-tree-sha1 = "b8e3a20ffbd00595667324af704b3e6ce310fcdd"
...
git-tree-sha1 = "99c4cfd40314e186f7688580980d3c36954067ae"
...
[[deps.Meshes]]
deps = ["Bessels", "CircularArrays", "Colorfy", "CoordRefSystems", "DelaunayTriangulation", "Distances", "LinearAlgebra", "NearestNeighbors", "Random", "Rotations", "ScopedValues", "SparseArrays", "StaticArrays", "StatsBase", "TransformsBase", "Unitful"]
git-tree-sha1 = "69c59bd1e2e22aabfb5bf102e2493ce2a646c0f9"

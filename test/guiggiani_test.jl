...
@testset "Laurent coefficients" begin
    f = ρ -> ρ^2 + 2ρ + 1
    f₋₂, f₋₁, f₀ = @inferred Inti.laurent_coefficients(f, 1.0e-2, Val(-2))
    @test norm((f₋₂, f₋₁, f₀) .- (0, 0, 1)) < 1.0e-10
    f₋₂, f₋₁, f₀ = @inferred Inti.laurent_coefficients(f, 1.0e-2, Val(0))
    @test norm((f₋₂, f₋₁, f₀) .- (0, 0, 1)) < 1.0e-10
    ...
    @test norm((f₋₂, f₋₁, f₀) .- (1, 1, 1.5)) < 1.0e-10

    f = ρ -> SVector(cos(ρ), sin(ρ)) / ρ^2 + SVector(exp(ρ), 0.2) / ρ
    f₋₂, f₋₁, f₀ = Inti.laurent_coefficients(f, 1.0e-1, Val(-2))
    ...
    @inferred Inti.laurent_coefficients(g, 1.0e-3, (Val(-2)))
end
...
@testset "Plane distorted element" begin
    ...
    @test isapprox(va, -5.749237; atol = 1.0e-4)
    @test isapprox(vb, -9.154585; atol = 1.0e-4)

...
println("Setting up Laplace gradient script parameters and geometry...")
...
u_exact = x -> -1 / (2π) * log(norm(x - source_point))
...
println("\n" * "="^30)
println("Running Laplace kernel-split gradient script")
println("="^30)
...
println("Laplace solution max absolute error: ", solution_abs_err)
println("Laplace solution max relative error: ", solution_relerr)
println("Laplace gradient max absolute error: ", abs_err)
println("Laplace gradient max relative error: ", relerr)

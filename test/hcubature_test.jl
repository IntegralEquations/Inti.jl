...
@testset "Segment integration" begin
    ...
    # not singular
    s = Inti.Point1D(2)
    fevals[] = 0; I1, E1 = Inti.hcubature(x -> f(x,s), τ̂; atol)
    @test fevals[] < 50
    @test norm(I(s) - I1) < atol
    # singular
    s = Inti.Point1D(1/3)
    fevals[] = 0; I1, E1 = Inti.hcubature(x -> f(x,s), τ̂; atol) # naive
    n1 = fevals[]
    @test norm(I(s) - I1) < atol
    fevals[] = 0; I1, E1 = Inti.hcubature(x -> f(x,s), τ̂, s; atol) #
    n2 = fevals[]
    @test norm(I(s) - I1) < atol
    @test n2 < n1
end
...
@testset "Triangle integration" begin
    ...
    Ie = 1 / 2 * (sin(1) - cos(1))
    fevals[] = 0; Ia, Ea = Inti.hcubature(f, τ̂)
    n1 = fevals[]
    @test Ie ≈ Ia
    # singular kernel at right vertex
    s = Inti.Point2D(1, 0)
    f = (x,s) -> (fevals[] += 1; 1/norm(x - s))
    fevals[] = 0; Ia, Ea = Inti.hcubature(x->f(x,s), τ̂)
    n1 = fevals[]
    Ie = acosh(sqrt(2))
    @test Ia ≈ Ie
    ...
    fevals[] = 0; I1, E1 = Inti.hcubature(x -> f(x,s), τ̂; atol)
    n1 = fevals[]
    fevals[] = 0; I2, E2 = Inti.hcubature(x -> f(x,s), τ̂, s; atol)
    n2 = fevals[]
    @test I1 ≈ I2
    @test n2 < n1
end
...
@testset "Square integration" begin
    ...
    f = x -> (fevals[] += 1; cos(x[1]) * sin(x[2]))
    Ie = -sin(1)*(cos(1) - 1)
    fevals[] = 0; Ia, Ea = Inti.hcubature(f, τ̂; atol)
    n1 = fevals[]

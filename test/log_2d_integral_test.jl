...
for _ in 1:levels
    ...
    EtoQnodes =
        Q.etype2qtags[Inti.LagrangeElement{Inti.ReferenceSimplex{2},10,SVector{2,Float64}}]
    elements = Q.mesh.etype2etags
    Els2Nodes = Q.mesh.parent.etype2mat[Inti.LagrangeElement{
    ...
    for ind_el in 1:size(Els2Nodes, 2)
        ...
        if inpolygon(SVector(0.0, 0.0), trinodes) == 1
            I_log =
                I_log - sum(
                    q -> norm(q.coords)^2 * log(norm(q.coords)) * q.weight,
                    Q.qnodes[EtoQnodes[:, ind_el]],
                )

...
nbdry_els = size(
    msh.etype2mat[Inti.LagrangeElement{Inti.ReferenceHyperCube{1},2,SVector{2,Float64}}],
)[2]
...
for E in Inti.element_types(msh)
    ...
    for elind in eachindex(els)
        ...
        if j > 1
            ...
            Jₖ_Z =
                (x) -> [
                    cₖ[1]-bₖ[1] + Φₖ_Z_der_x1(x)[1]; aₖ[1]-bₖ[1] + Φₖ_Z_der_x2(x)[1];;
                    cₖ[2]-bₖ[2] + Φₖ_Z_der_x1(x)[2]; aₖ[2]-bₖ[2] + Φₖ_Z_der_x2(x)[2]
                ]
            # loop over entities

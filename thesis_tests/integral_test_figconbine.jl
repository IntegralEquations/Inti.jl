...
row = 0
for (α, β) in ((1, 0), (0, 1)) # coefficients for single, double layer
...
op = α == 1 ? "S" : "D"
i = Dirichlet ? 0 : 1
ax1 = Axis(fig[row, 2], xticks=(H[1:5], [Lpower(2, -i) for i in ii]))
ax2 = Axis(fig[row, 1], xticks=(H[6:9], [L"10^{-4}", L"2\times10^{-4}", L"2^{2}\times10^{-4}", L"2^{3}\times10^{-4}"]), ylabel=L"\text{Error of }%$(op)[\gamma_%$i\sigma](\mathbf{x}_t)")
if row != 4
...
SAVE && save("thesis_tests/integral_test_plots/$(GEOM)_integral_error.png", fig)